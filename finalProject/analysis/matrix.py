...
def constructMatrix():
    ...
    profitYesOrNo = stats.profit_classifier(myDict, netProfit)
    ...
    allDictsYesOrNo= [
        ...
        ("Number of Shows", numberOfShows),
        ("Number of Stands", numberOfStands),
        ("Profit Yes or No", profitYesOrNo)
    ]
    ...
    matrixYesOrNo = pd.DataFrame()
    for name, stat_dict in allDictsYesOrNo:
        stat_series = pd.Series(stat_dict, name = name)
        matrixYesOrNo = pd.concat([matrixYesOrNo, stat_series], axis = 1)
    
    matrixYesOrNo = matrixYesOrNo.fillna(0)

    return matrixAmount, matrixYesOrNo